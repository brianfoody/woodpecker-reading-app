  ...
  try {
    ...
    for (let i = 0; i < paragraphs.length; i += PARAGRAPH_BATCH_SIZE) {
      ...
      const batchPromises = batch.map(
        async (paragraphText: string, batchIndex: number) => {
          const pIndex = batchStartIndex + batchIndex;
          try {
            const response =
              await elevenlabs.textToSpeech.convertWithTimestamps(voiceId, {
                text: paragraphText,
                modelId: "eleven_v3",
              });
            ...
            console.error(
              `Failed to generate audio for paragraph ${pIndex}:`,
              error
            );
            throw error;
          }
    ...
    return NextResponse.json({
      ...
      words:
        mainWords.length > 0
          ? mainWords
          : processedParagraphs.flatMap((p) => p.words),
